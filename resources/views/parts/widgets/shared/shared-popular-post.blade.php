...
<section class="box popular-post">
	...
	<article class="popular-post__wrapper">
		@if (!empty($post->image))
			<a class="popular-post__image-link" href="{{ route('site.post.post', $post->slug) }}" class="image featured">
				<img class="popular-post__image" src="{{ $post->image }}" alt="">
			</a>
		@endif
		<h3 class="popular-post__title">
			<a href="{{ route('site.post.post', $post->slug) }}">{{ $post->title }}</a>
		</h3>
		...
		@if (mb_strlen($post->content) >= 200)
			<p class="popular-post__more">
				<a class="popular-post__more-link" href="{{ route('site.post.post', $post->slug) }}">
